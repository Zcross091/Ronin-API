...
import { fetchFromMiruro, fetchFromAnify } from '../utils/fallback';
      ...
      try {
        const fetchWithFallback = async () => {
          try {
            ...
            console.error(`[Failsafe] Gogoanime scraper failed for ${episodeId}. Trying Miruro-API...`);
            try {
              return await fetchFromMiruro(episodeId);
            } catch (miruroError) {
              console.error(`[Failsafe] Miruro-API failed for ${episodeId}. Trying Anify API...`);
              return await fetchFromAnify(episodeId);
            }
          }
        };
          ...
          ? await cache.fetch(
              ...
              async () =>
                await fetchWithFallback()
                  .catch((err) => reply.status(404).send({ message: err.message || err })),
              redisCacheTime,
            )
          : await fetchWithFallback()
              .catch((err) => reply.status(404).send({ message: err.message || err }));
